...
class PomodoroModule {
    constructor() {
        this.storageKey = 'pomodoro_state';
        this.settingsKey = 'pomodoro_settings';
        this.historyKey = 'pomodoro_history';
    ...
    /**
     * One-time migration: this module used to write directly to localStorage
     * under the app's old "lifeflow_" name, bypassing StorageManager entirely.
     * That meant Pomodoro settings/state/history were silently excluded from
     * Export Backup, Import Backup, and the storage health check. This copies
     * any existing old-format data into StorageManager (flowora_ prefix) once,
     * then removes the old raw keys so there's a single source of truth.
     */
    migrateLegacyStorage() {
        try {
            const legacyState = localStorage.getItem('lifeflow_pomodoro_state');
            if (legacyState !== null && StorageManager.get(this.storageKey) === null) {
                StorageManager.set(this.storageKey, JSON.parse(legacyState));
            }
            const legacySettings = localStorage.getItem('lifeflow_pomodoro_settings');
            if (legacySettings !== null && StorageManager.get(this.settingsKey) === null) {
                StorageManager.set(this.settingsKey, JSON.parse(legacySettings));
            }
            const legacyHistory = localStorage.getItem('lifeflow_pomodoro_history');
            if (legacyHistory !== null && StorageManager.get(this.historyKey) === null) {
                StorageManager.set(this.historyKey, JSON.parse(legacyHistory));
            }
            ['lifeflow_pomodoro_state', 'lifeflow_pomodoro_settings', 'lifeflow_pomodoro_history'].forEach(k => {
                localStorage.removeItem(k);
            });
        } catch (e) {
            console.warn('[PomodoroModule] Legacy storage migration skipped:', e);
        }
    }
        ...
        this.timerId = setInterval(() => {
            ...
            if (deltaSeconds >= 1) {
                ...
                if (this.timeLeft <= 0) {
                    ...
                    if (typeof FloworaApp !== 'undefined' && typeof FloworaApp.triggerLocalNotification === 'function') {
                        const notifTitle = this.isWorkSession ? 'Focus session complete!' : 'Break time is over!';
                        ...
                        FloworaApp.triggerLocalNotification(notifTitle, notifBody);
    ...
    updateDisplay() {
        ...
        if (this.isRunning) {
            document.title = `(${display.textContent}) ${this.currentTask} | Flowora`;
        } else {
            document.title = `Flowora - Productivity & Life Planner`;
        }
    }
    ...
    restoreState() {
        const state = StorageManager.get(this.storageKey);
        if (!state) return;
    ...
    loadSettings() {
        return StorageManager.get(this.settingsKey, {});
    }
    ...
    saveSettings() {
        ...
        StorageManager.set(this.settingsKey, settings);
    }

    logCompletedSession() {
        try {
            const history = StorageManager.get(this.historyKey, []);
            history.push({
                task: this.currentTask,
                durationMinutes: this.workTime / 60,
                timestamp: new Date().toISOString()
            });
            StorageManager.set(this.historyKey, history);
        } catch (e) {
    ...
    playAmbientSound(type) {
        ...
        try {
            ...
            console.warn('Ambient sound generation error:', e);
        }
    }
