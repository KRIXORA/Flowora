...
class GoalsModule {
    constructor() {
        ...
        this.goals = StorageManager.get(this.storageKey, [
            { id: 'goal_1', title: 'Read 12 books this year', progress: 40, category: 'Learning', dueDate: '2026-12-31' },
            { id: 'goal_2', title: 'Save ₹50,000 for an emergency fund', progress: 65, category: 'Finance', dueDate: '2026-12-15' },
            { id: 'goal_3', title: 'Run a 5K without stopping', progress: 30, category: 'Health', dueDate: '2026-10-01' }
        ]);
    ...
    initSortable() {
        ...
        if (container) {
            if (container.__sortableInstance) {
                container.__sortableInstance.destroy();
            }
            container.__sortableInstance = Sortable.create(container, {
                animation: 150,
                onEnd: (evt) => {
                    const movedId = evt.item.getAttribute('data-goal-id');
                    if (!movedId) return;

                    const fromGlobalIdx = this.goals.findIndex(g => g.id === movedId);
                    if (fromGlobalIdx === -1) return;
                    const [movedItem] = this.goals.splice(fromGlobalIdx, 1);

                    // Anchor the new position to whichever card now sits right after
                    // the dragged one in the (possibly filtered) visible list, so
                    // reordering stays correct even when a search/category filter
                    // is active and only shows a subset of this.goals.
                    const nextSibling = evt.item.nextElementSibling;
                    let insertAt = this.goals.length;
                    if (nextSibling) {
                        const nextId = nextSibling.getAttribute('data-goal-id');
                        const nextGlobalIdx = this.goals.findIndex(g => g.id === nextId);
                        if (nextGlobalIdx !== -1) insertAt = nextGlobalIdx;
                    }

                    this.goals.splice(insertAt, 0, movedItem);
                    this.persistState();
                    this.renderGoals();
                    this.initSortable();
                }
    ...
    renderGoalsWrapper() {
        ...
        document.getElementById('goalSearchInput')?.addEventListener('input', Utils.debounce((e) => {
    ...
    renderGoals() {
        ...
        if (Array.isArray(filteredGoals) && filteredGoals.length > 0) {
            ...
            const hasAnyGoals = this.goals.length > 0;
            container.innerHTML = `
                <div style="text-align: center; padding: 40px; color: var(--text-muted); font-size: 0.85rem; border: 1px dashed var(--border-glass); border-radius: var(--radius-md); background: var(--bg-main);">
                    ...
                    ${hasAnyGoals ? 'No goals match your filter or search.' : 'No goals yet. Click "Add Goal" to set your first one.'}
                </div>
            `;
        }
    }
    ...
    openGoalModal(editIndex = null) {
        ...
        const targetGoal = isEditing ? this.goals[editIndex] : { title: '', category: this.categories[0]?.id || 'Personal', progress: 0, dueDate: '' };
        ...
        window.componentManager = window.componentManager || new ComponentManager();
        componentManager.openModal(isEditing ? 'Edit Goal' : 'Add New Goal', modalHTML);
    ...
    deleteGoal(index) {
        if (this.goals[index]) {
            const [removedGoal] = this.goals.splice(index, 1);
            this.persistState();
            this.renderGoalsWrapper();
            this.renderGoals();
            this.initSortable();
            ComponentManager.showUndoToast(`"${removedGoal.title}" deleted.`, () => {
                this.goals.splice(index, 0, removedGoal);
                this.persistState();
                this.renderGoalsWrapper();
                this.renderGoals();
                this.initSortable();
                ComponentManager.showToast('Goal restored.', 'success');
            });
        }
    }
