...
class HabitsModule {
    ...
    /**
     * Habits previously had no concept of "which day" completedToday referred
     * to, so once marked done it stayed shown as completed forever (it never
     * reset for a new day), and streaks never broke even if a day was missed
     * entirely. This runs once per load: rolls completedToday back to false
     * for a new day, and resets streak to 0 if more than one full day was
     * skipped since the habit was last completed.
     */
    checkDailyReset() {
        ...
        this.habits.forEach(habit => {
            if (!habit.lastCompletedDate) {
                // Pre-existing habit from before this field existed — seed it
                // from its current flag so behavior is correct going forward.
                habit.lastCompletedDate = habit.completedToday ? todayStr : null;
                return;
            }

            if (habit.lastCompletedDate === todayStr) return; // already current

            const diffDays = Math.round((new Date(todayStr) - new Date(habit.lastCompletedDate)) / 86400000);
            ...
            if (diffDays > 1) {
                // A full day (or more) was skipped entirely — streak is broken.
                habit.streak = 0;
                changed = true;
            }
        });
    ...
    renderHabits() {
        ...
        document.getElementById('habitSearchInput')?.addEventListener('input', Utils.debounce((e) => {
    ...
    toggleHabit(index) {
        ...
        if (this.habits[index].completedToday) {
            ...
            this.habits[index].lastCompletedDate = new Date().toDateString();
            ...
            this.habits[index].lastCompletedDate = null;
