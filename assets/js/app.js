...
class FloworaApp {
    ...
    /**
     * Phase 1: Catch unhandled errors so the app doesn't silently break
     */
    setupGlobalErrorHandling() {
        window.addEventListener('error', (event) => {
            console.error('[Flowora] Uncaught error:', event.error || event.message);
            if (typeof ComponentManager !== 'undefined') {
                ComponentManager.showToast('Something went wrong. Check the console for details.', 'error');
            }
        });

        window.addEventListener('unhandledrejection', (event) => {
            console.error('[Flowora] Unhandled promise rejection:', event.reason);
            if (typeof ComponentManager !== 'undefined') {
                ComponentManager.showToast('An unexpected error occurred.', 'error');
            }
        });

        // Listen for storage quota warnings from StorageManager
        window.addEventListener('floworaStorageQuota', (e) => {
            if (typeof ComponentManager !== 'undefined') {
                ComponentManager.showToast(e.detail?.message || 'Storage is almost full. Please export a backup.', 'error');
            }
        });
    }

    /**
     * Phase 1: Gently remind user to export a backup if none in last 14 days
     */
    checkBackupReminder() {
        try {
            if (typeof StorageManager === 'undefined') return;
            const lastBackup = StorageManager.get('last_backup_at', null);
            const now = Date.now();
            const fourteenDays = 14 * 24 * 60 * 60 * 1000;

            if (!lastBackup || (now - new Date(lastBackup).getTime()) > fourteenDays) {
                // Delay so it doesn't fight with other toasts on first load
                setTimeout(() => {
                    if (typeof ComponentManager !== 'undefined') {
                        ComponentManager.showToast('Tip: Export a backup from Settings to keep your data safe.', 'info');
                    }
                }, 4000);
            }
        } catch (e) {
    ...
    init() {
        ...
        if (paletteInput) {
            paletteInput.addEventListener('input', Utils.debounce((e) => {
    ...
    searchWorkspace(query) {
        ...
        try {
            ...
            console.error('[FloworaApp] Search failed:', error);
        }
