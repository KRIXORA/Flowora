...
class PlannerModule {
    constructor() {
        ...
        this.timeSlots = StorageManager.get(this.slotsStorageKey, [
            { id: 'morning', label: 'Morning', time: '06:00 - 12:00', icon: 'fa-mug-hot' },
            { id: 'afternoon', label: 'Afternoon', time: '12:00 - 18:00', icon: 'fa-sun' },
            { id: 'evening', label: 'Evening', time: '18:00 - 22:00', icon: 'fa-moon' }
        ]);

        this.plannerTasks = StorageManager.get(this.storageKey, {
            morning: [
                { id: 'task_m1', title: 'Plan your day & review priorities', completed: false, scheduled: true, priority: 'high', duration: '15 mins', date: this.selectedDate, subtasks: [{text: 'Check calendar', done: true}, {text: 'Set top 3 priorities', done: false}], createdAt: new Date().toISOString() }
            ],
            afternoon: [
                { id: 'task_a1', title: 'Focused work block', completed: false, scheduled: true, priority: 'critical', duration: '2 hrs', date: this.selectedDate, subtasks: [], createdAt: new Date().toISOString() }
            ],
            evening: [
                { id: 'task_e1', title: 'Wrap up & reflect on the day', completed: false, scheduled: true, priority: 'medium', duration: '30 mins', date: this.selectedDate, subtasks: [], createdAt: new Date().toISOString() }
            ]
        });
    ...
    bindGlobalSyncEvents() {
        window.addEventListener('storage', (e) => {
            if (e.key === StorageManager.PREFIX + this.storageKey) {
                this.plannerTasks = StorageManager.get(this.storageKey, {});
                this.renderPlanner();
                this.initSortable();
            }
    ...
    openManageSlotsModal() {
        ...
        document.getElementById('addNewSlotRowBtn').addEventListener('click', () => {
            this.timeSlots.push({ id: Utils.uid('slot'), label: 'Custom Block', time: 'Custom', icon: 'fa-clock' });
        ...
        const modalHTML = `
            <div style="display: flex; flex-direction: column; gap: 14px; max-height: 75vh; overflow-y: auto; padding-right: 4px;">
                <div class="form-group" style="display: flex; flex-direction: column; gap: 6px;">
                    ...
                    <input type="text" id="modalTaskTitle" placeholder="e.g., Team meeting at 3 PM" autocomplete="off" style="padding: 12px; border-radius: var(--radius-md); border: 1px solid var(--border-glass); background: var(--bg-main); color: var(--text-primary); font-size: 0.9rem;">
                </div>
        ...
        const tick = () => {
            ...
            if (totalSeconds <= 0) {
                ...
                try {
                    ...
                    StorageManager.set('pomodoro_history', history);
                } catch (e) {
                    console.error('[PlannerModule] Failed to log timer session:', e);
                }
    ...
    removeTask(slot, globalIndex) {
        if (this.plannerTasks[slot] && this.plannerTasks[slot][globalIndex]) {
            const [removedTask] = this.plannerTasks[slot].splice(globalIndex, 1);
            this.persistState();
            this.renderPlanner();
            this.initSortable();
            ComponentManager.showUndoToast(`"${removedTask.title}" deleted.`, () => {
                this.plannerTasks[slot].splice(globalIndex, 0, removedTask);
                this.persistState();
                this.renderPlanner();
                this.initSortable();
