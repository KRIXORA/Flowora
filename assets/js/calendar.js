...
class CalendarModule {
    ...
    bindStorageListener() {
        window.addEventListener('storage', (e) => {
            if (e.key === StorageManager.PREFIX + this.storageKey && e.newValue) {
                try {
    ...
    exportCalendarData() {
        ...
        downloadAnchor.setAttribute("download", `flowora_calendar_backup_${new Date().toISOString().slice(0, 10)}.json`);
    ...
    importCalendarData(event) {
        ...
        reader.onload = (e) => {
            const parsed = Utils.safeJSONParse(e.target.result);
            if (parsed && typeof parsed === 'object') {
                ...
                console.error('[CalendarModule] Import failed: invalid or unparsable backup file');
