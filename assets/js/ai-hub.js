...
class AIHubModule {
    constructor() {
        ...
        this.modelKey = 'ai_model';
        this.migrateLegacyStorage();
        this.selectedModel = StorageManager.get(this.modelKey, 'gemini-1.5-flash');
        this.messages = StorageManager.get(this.storageKey, [
            {
                sender: 'bot',
                text: 'Hi! I am your Flowora Assistant. Tell me what you would like to do — add a task, check your progress, or plan your day.',
    ...
    /**
     * This module used to double-prefix its chat-history key (storing it as
     * "flowora_" + "flowora_ai_chat_history" once StorageManager added its
     * own prefix) and read the model preference from an old "lifeflow_"
     * key name bypassing the app's current naming. Neither broke the app by
     * itself, but both showed up oddly in exported backups. This copies any
     * existing data across once, under the corrected key names.
     */
    migrateLegacyStorage() {
        try {
            const legacyChat = StorageManager.get('flowora_ai_chat_history');
            if (legacyChat !== null && StorageManager.get(this.storageKey) === null) {
                StorageManager.set(this.storageKey, legacyChat);
            }
            const legacyModel = localStorage.getItem('lifeflow_ai_model');
            if (legacyModel !== null && StorageManager.get(this.modelKey) === null) {
                StorageManager.set(this.modelKey, JSON.parse(legacyModel));
            }
            localStorage.removeItem('lifeflow_ai_model');
            localStorage.removeItem('flowora_flowora_ai_chat_history');
        } catch (e) {
            console.warn('[AIHubModule] Legacy storage migration skipped:', e);
        }
    }
            ...
            <div class="glass-card ai-hub-card" style="display: flex; flex-direction: column; height: calc(100vh - 160px); min-height: 550px; padding: 24px;">
                <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid var(--border-glass); padding-bottom: 16px; margin-bottom: 16px; flex-wrap: wrap; gap: 12px;">
                    <div style="display: flex; align-items: center; gap: 12px;">
                        ...
                        <div>
                            <h2 style="font-size: 1.2rem; font-weight: 600;">Flowora Executive Coach</h2>
    ...
    bindEvents() {
        ...
        if (modelSelector) {
            modelSelector.addEventListener('change', (e) => {
                ...
                StorageManager.set(this.modelKey, this.selectedModel);
    ...
    async processDynamicAIResponse(query) {
        ...
        const typingId = Utils.uid('typing');
        ...
        if (apiKey && apiKey.startsWith('AIza')) {
            try {
                ...
                if (data.candidates && data.candidates[0].content.parts[0].text) {
                    ...
                    botReply = 'Received an empty response from the Gemini API.';
                }
            } catch (err) {
                console.error(err);
                botReply = 'Error connecting to the Gemini API. Falling back to the internal intelligence engine.\n\n' + this.generateSmartReply(query);
            }
        } else {
    ...
    exportChatJSON() {
        ...
        downloadAnchor.setAttribute("download", `flowora_ai_chat_${Utils.fileTimestamp()}.json`);
